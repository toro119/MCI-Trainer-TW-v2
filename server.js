...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (payload, callback) => {
    ...
    socket.data.role = payload.role || "primary";
    room.members.push({
      socketId: socket.id,
    ...
    addLog(room, `${socket.data.name} 以「${socket.data.role}」加入演練`);
    callback({ ok: true, state: publicState(room) });
    broadcast(room);
  });


  socket.on("startExercise", (_, callback) => {
    const room = currentRoom(socket);
    if (!room) return callback && callback({ ok: false, message: "房間不存在" });
    if (room.exercise.status === "running") return callback && callback({ ok: true });
    room.exercise.status = "running";
    room.exercise.startedAt = Date.now();
    room.exercise.pausedAt = null;
    room.exercise.pausedTotalMs = 0;
    room.exercise.endedAt = null;
    addLog(room, "教官開始演練");
    callback && callback({ ok: true });
    broadcast(room);
  });

  socket.on("pauseExercise", (_, callback) => {
    const room = currentRoom(socket);
    if (!room) return callback && callback({ ok: false, message: "房間不存在" });
    if (room.exercise.status !== "running") return callback && callback({ ok: false, message: "演練目前不是進行中" });
  ...
  socket.on("resumeExercise", (_, callback) => {
    const room = currentRoom(socket);
    if (!room) return callback && callback({ ok: false, message: "房間不存在" });
    if (room.exercise.status !== "paused") return callback && callback({ ok: false, message: "演練目前不是暫停狀態" });
  ...
  socket.on("endExercise", (_, callback) => {
    const room = currentRoom(socket);
    if (!room) return callback && callback({ ok: false, message: "房間不存在" });
  ...
  socket.on("lockPatient", ({ patientId }, callback) => {
    ...
    if (socket.data.role === "primary" && !patient.primaryStartedAt) patient.primaryStartedAt = Date.now();
    if (socket.data.role === "secondary" && !patient.secondaryStartedAt) patient.secondaryStartedAt = Date.now();
    callback({ ok: true });
  ...
  socket.on("submitPrimary", ({ patientId, result }, callback) => {
    ...
    if (patient.primaryStartedAt) {
      patient.primaryDurationSec = Math.max(1, Math.round((Date.now() - patient.primaryStartedAt) / 1000));
    }
    patient.lockedBy = null;
    patient.lockedByName = "";
    addLog(room, `${patient.id} 一次檢傷：${result}｜${patient.primaryDurationSec || 0}秒`);
    callback({ ok: true });
    broadcast(room);
  });
  ...
  socket.on("submitSecondary", ({ patientId, result }, callback) => {
    ...
    if (patient.secondaryStartedAt) {
      patient.secondaryDurationSec = Math.max(1, Math.round((Date.now() - patient.secondaryStartedAt) / 1000));
    }
    patient.lockedBy = null;
    patient.lockedByName = "";
    addLog(room, `${patient.id} 二次檢傷：${result}｜${patient.secondaryDurationSec || 0}秒`);
    callback({ ok: true });
    broadcast(room);
  });
