...
function makePatients(count) {
  return Array.from({ length: count }, (_, index) => ({
    ...
    radialPulseDescription: PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length].pulse
      ...
      : "雙側橈動脈觸診約5至10秒，皆無法觸及。",
    injuryRegions: (() => {
      const t = PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length];
      const text = `${t.note} ${t.mechanism} ${t.signs}`;
      const regions = [];
      if (/頭|額|鼻|頸/.test(text)) regions.push("head");
      if (/胸|呼吸|氣胸|吸入/.test(text)) regions.push("chest");
    ...
    injuryIcons: (() => {
      ...
      const text = `${t.note} ${t.mechanism} ${t.signs}`;
      const icons = [];
      if (/出血|割傷|撕裂傷|穿刺傷/.test(text)) icons.push("bleeding");
      if (/骨折|變形/.test(text)) icons.push("fracture");
      if (/灼傷|燒傷|濃煙/.test(text)) icons.push("burn");
    ...
    posture: (() => {
      const t = PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length];
      if (t.walk) return "standing";
      if (!t.command) return "supine";
      if (/夾擊|擠壓|受困/.test(`${t.note} ${t.mechanism}`)) return "trapped";
      return "sitting";
    })(),
    triageProtocol: (
      PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length].age <= 8
      || String(PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length].sex).includes("童")
    ) ? "jumpstart" : "start",
    massiveBleeding: /大量出血|噴射性出血|股動脈/.test(
      PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length].note
    ),
    ...
    identity: (() => {
      const t = PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length];
      if (index % 17 === 5 && String(t.sex).includes("女")) return "pregnant";
      if (index % 19 === 7) return "rescuer";
      return "civilian";
    })(),
    identityLabel: (() => {
      const t = PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length];
      if (index % 17 === 5 && String(t.sex).includes("女")) return "孕婦";
      if (index % 19 === 7) return "救災救護人員";
      return "一般民眾";
    })(),
    specialPriorityRed: (() => {
      const t = PATIENT_TEMPLATES[index % PATIENT_TEMPLATES.length];
      return (index % 17 === 5 && String(t.sex).includes("女")) || index % 19 === 7;
    })(),
    airwayOpenCount: 0,
    primaryResult: null,
    primaryBy: "",
