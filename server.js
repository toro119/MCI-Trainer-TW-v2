...
io.on("connection", (socket) => {
  socket.on("createRoom", (payload, callback) => {
    ...
    room.members.push({ socketId: socket.id, name: socket.data.name, role: "instructor", roles: ["instructor"], activePatientId: null });
  ...
  socket.on("lockPatient", ({ patientId }, callback) => {
    ...
    const activeMember = room.members.find((item) => item.socketId === socket.id);
    if (activeMember) activeMember.activePatientId = patient.id;
  ...
  socket.on("cancelTriage", ({ patientId }, callback) => {
    ...
    if (patient.lockedBy === socket.id) {
      ...
      const cancelMember = room.members.find((item) => item.socketId === socket.id);
      if (cancelMember) cancelMember.activePatientId = null;
    }
    callback && callback({ ok:true });
    broadcast(room);
  });
  ...
  socket.on("submitPrimary", ({ patientId, result }, callback) => {
    ...
    patient.lockedBy = null;
    patient.lockedByName = "";
    const primaryMember = room.members.find((item) => item.socketId === socket.id);
    if (primaryMember) primaryMember.activePatientId = null;
  ...
  socket.on("submitSecondary", ({ patientId, result }, callback) => {
    ...
    patient.lockedBy = null;
    patient.lockedByName = "";
    const secondaryMember = room.members.find((item) => item.socketId === socket.id);
    if (secondaryMember) secondaryMember.activePatientId = null;
